...
def init_bars_db(target = 0):
	...
	try:
		...
		if minute:
			...
			collection = store.collection("1MIN",overwrite=True,npartitions=1 )
		if day:
			store.delete_collection("1DAY")
			collection_day = store.collection("1DAY",overwrite=True,npartitions=1)
		...
		print(date)
		df = pd.DataFrame([{"ticker":"ticker","open":0.1,"close":0.1,"high":0.1,"low":0.1,"volume":0.1,"trades":0,"time":date,"timestamp":date}])
