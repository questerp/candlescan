...
def init_bars_db(target = 0):
	...
	try:
		...
		df = pd.DataFrame([{"ticker":"NaN","open":0.1,"close":0.1,"high":0.1,"low":0.1,"volume":0.0,"trades":0,"time":dt.now()}])
		df  = df.as_type({"ticker":"str","open":"float64","close":"float64","high":"float64","low":"float64","volume":"float64","trades":"int32","time":"datetime"})
		df.set_index("time",inplace=True,drop=True)
		print(df.info())
		for s in symbols:
			#if s not in items:
