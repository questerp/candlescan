...
class Share(object):
    ...
    def get_historical(self, period_type, period, frequency_type, frequency,mode="cascade"):
        ...
        if mode=="cascade":
            ...
            return_data = [{'timestamp':data['timestamp'][a]*1000,'open':quotes['open'][a],'close':quotes['close'][a],'high':quotes['high'][a],'low':quotes['low'][a],'volume':quotes['volume'][a]} for a in range(len(data['timestamp']))]
        else:
